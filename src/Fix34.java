...
public class Fix34 {
	...
	public static void main(String[] args) {
		...
		int[] nums34 = fix34(nums);

		for (int i = 0; i < nums.length; i++) {
			System.out.print(nums34[i] + " ");
		}
	}

	public static int[] fix34(int[] nums) {

		ArrayList<Integer> foursPosition = new ArrayList<Integer>();
		int currentFour = 0;
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] == 4) {
				foursPosition.add(i);
		...
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] == 3) {
				int temp = nums[i + 1];
				nums[i + 1] = nums[foursPosition.get(currentFour)];
				nums[foursPosition.get(currentFour)] = temp;
				currentFour++;
			}
		}
